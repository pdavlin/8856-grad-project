...
function makeFetch(endpoint: string, opts: CouchOptions = {}): Fetcher {
  return (
    path: string,
    {
      method = "GET",
...
class CouchDatabase<T> {
  ...
  async insert(
    ...
    const body = JSON.stringify(doc).replace(/"/g, '"');
  ...
  async getView<T>(docName: string, viewName: string, key?: string) {
    const res = await this.fetch(
      `/_design/${docName}/_view/${viewName}${key ? `?key=%22${key.split(' ').join('%20')}%22` : ""}`,
      {
        method: "GET",
        headers: new Headers({
          accept: "application/json",
